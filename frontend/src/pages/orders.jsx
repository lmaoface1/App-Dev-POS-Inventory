  ...
  const toggleRow = async (saleId) => {
    ...
    if (!detail[saleId]) {
      try {
        ...
        setDetail((prev) => ({ ...prev, [saleId]: res.data }));
      } catch (err) {
        console.error(err);
      }
    }
  ...
  return (
    <div className="p-8 space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-400 mt-0.5">All processed sales transactions</p>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100">
                {['Sale ID', 'Cashier', 'Total Amount', 'Date', 'Details'].map((h) => (
            ...
            <tbody>
              {filtered.map((sale) => (
                <>
                  ...
                  >
                    <td className="py-3.5 pr-4 font-mono text-gray-600">#{sale.id}</td>
                    <td className="py-3.5 pr-4 text-gray-800">{sale.cashier_name || `User #${sale.cashier_id}`}</td>
                    <td className="py-3.5 pr-4 font-bold text-gray-900">₱{Number(sale.total_amount).toFixed(2)}</td>
                            ...
                            <table className="w-full text-sm">
                              <thead>
                                <tr className="border-b border-gray-200">
                                  <th className="text-left text-gray-400 font-medium pb-2">Product</th>
                                  <th className="text-right text-gray-400 font-medium pb-2">Qty</th>
                                  <th className="text-right text-gray-400 font-medium pb-2">Unit Price</th>
                                  <th className="text-right text-gray-400 font-medium pb-2">Subtotal</th>
                                </tr>
                              </thead>
                              <tbody>
                                {(detail[sale.id].items || []).map((item) => (
                                  <tr key={item.id}>
                                    <td className="py-2 text-gray-800">{item.product_name || `Product #${item.product_id}`}</td>
                                    <td className="py-2 text-right text-gray-600">{item.quantity}</td>
                                    <td className="py-2 text-right text-gray-600">₱{Number(item.unit_price).toFixed(2)}</td>
                                    <td className="py-2 text-right font-medium text-gray-800">₱{(item.quantity * item.unit_price).toFixed(2)}</td>
                                  </tr>
                                ))}
                              </tbody>
                            </table>
